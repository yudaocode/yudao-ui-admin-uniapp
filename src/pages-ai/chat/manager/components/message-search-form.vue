  ...
  >
    <view class="yd-search-form-container">
      <view class="yd-search-form-item">
        ...
        <wd-input v-model="formData.conversationId" type="number" placeholder="请输入对话编号" clearable />
      </view>
      <view class="yd-search-form-item">
        <view class="yd-search-form-label">
          消息内容
...
const formData = reactive({
  conversationId: props.conversationId ? String(props.conversationId) : undefined as string | undefined,
...
const placeholder = computed(() => {
  ...
  if (formData.conversationId) {
    conditions.push(`对话编号:${formData.conversationId}`)
  }
  if (formData.content) {
  ...
  return conditions.length > 0 ? conditions.join(' | ') : '搜索聊天消息'
})
...
function handleSearch() {
  visible.value = false
  emit('search', {
    conversationId: formData.conversationId ? Number(formData.conversationId) : undefined,
...
function handleReset() {
  formData.conversationId = undefined
