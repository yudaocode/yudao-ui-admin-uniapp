...
const emit = defineEmits<{
  ...
  (e: 'change', items: Product[]): void
}>()

const productOptions = ref<Product[]>([]) // 产品选项
const selectedValue = ref<PickerValue | ''>(props.type === 'checkbox' ? [] : '') // 当前选中值
const pickerRef = ref<SelectPickerInstance>() // 产品选择器
const resolvedColumns = computed<Record<string, any>[]>(() => props.columns ?? productOptions.value)
...
async function loadOptions() {
  ...
  productOptions.value = await getSimpleProductList(props.deviceType)
}
...
function handleConfirm({ value }: { value: PickerValue }) {
  ...
  const ids = Array.isArray(nextValue) ? nextValue : nextValue == null ? [] : [nextValue]
  emit('change', resolvedColumns.value.filter(item => item.id != null && ids.some(id => String(id) === String(item.id))) as Product[])
}
