<template>
  ...
  <wd-popup v-model="visible" position="top" :custom-style="getTopPopupStyle()" :modal-style="getTopPopupModalStyle()" @close="visible = false">
    <view class="yd-search-form-container">
      ...
      <yd-search-picker v-model="formData.status" label="设备状态" :dict-type="DICT_TYPE.MES_DV_MACHINERY_STATUS" all-option />
...
const formData = reactive<Record<string, any>>({ code: '', name: '', status: -1 }) // 搜索表单数据
const placeholder = computed(() => { // 搜索条件展示文案
  ...
  if (formData.status !== -1) {
    conditions.push(`状态:${getDictLabel(DICT_TYPE.MES_DV_MACHINERY_STATUS, formData.status)}`)
  }
