<template>
  <wd-cell v-if="showCell" :title="label" :value="displayValue" is-link center @click="open" />
  ...
  >
    <view class="h-full flex flex-col overflow-hidden bg-[#f5f5f5]">
      ...
      <scroll-view class="min-h-0 flex-1" scroll-y>
        ...
        <wd-empty v-if="filteredFriends.length === 0" icon="search" :tip="keyword ? '没有匹配的好友' : '暂无好友'" />
      </scroll-view>
    </view>
  </wd-popup>
</template>
...
const displayValue = computed(() => { // 表单行展示文案
  return selectedCount.value > 0 ? `已选择 ${selectedCount.value} 人` : '请选择'
})
