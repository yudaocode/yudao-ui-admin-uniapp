<template>
  <view class="rtc-page">
    ...
    <view class="relative z-2 flex flex-1 flex-col items-center px-48rpx pt-[calc(100rpx+env(safe-area-inset-top))]">
      <ImAvatar :src="peerAvatar" :name="peerName" size="168rpx" />
      ...
      <scroll-view v-if="isGroup && participantMembers.length" scroll-x class="mt-30rpx max-w-full whitespace-nowrap">
        <view class="inline-flex gap-22rpx px-16rpx">
          <view v-for="item in participantMembers" :key="item.userId" class="w-104rpx flex flex-col items-center">
            <view class="relative">
              <ImAvatar :src="item.avatar" :name="getMemberName(item)" size="76rpx" />
              <view
                class="absolute bottom-0 right-0 h-18rpx w-18rpx border-3rpx border-[#263239] rounded-full"
                :class="isJoined(item.userId) ? 'bg-[#07c160]' : 'bg-[#f0ad4e]'"
              />
            </view>
            <text class="mt-8rpx w-full truncate text-center text-21rpx text-white/80">{{ getMemberName(item) }}</text>
          </view>
        </view>
      </scroll-view>
    </view>
...
import { ImConversationType, ImRtcCallMediaType, ImRtcCallStage } from '@/utils/constants'
...
const { stage, call, peerName, peerAvatar, startedAt, accept, reject, hangup, invite } = useImRtc()
const groupMembers = ref<ImGroupMemberRespVO[]>([]) // 群通话成员
...
/** 是否视频通话 */
const isVideo = computed(() => (call.value?.mediaType || useImRtc().incoming.value?.mediaType) === ImRtcCallMediaType.VIDEO)
const {
  micEnabled,
  cameraEnabled,
  speakerEnabled,
...
const participantUserIds = computed(() => Array.from(new Set([
  ...(call.value?.joinedUserIds || []),
  ...(call.value?.inviteeIds || []),
  call.value?.inviterId,
  currentUserId.value,
].filter(Boolean) as number[]))) // 已加入或邀请中的成员
const participantMembers = computed(() => participantUserIds.value.map((userId) => {
  const member = groupMembers.value.find(item => item.userId === userId)
  return member || { id: userId, groupId: call.value?.groupId || 0, userId, displayUserName: `用户 ${userId}` }
})) // 通话成员展示列表
const inviteCandidates = computed(() => groupMembers.value
  .filter(item => !item.quitTime && item.userId !== currentUserId.value && !participantUserIds.value.includes(item.userId))
...
/** 判断成员是否已经接入 */
function isJoined(userId: number) {
  return userId === currentUserId.value || (call.value?.joinedUserIds || []).includes(userId)
}
...
async function loadGroupMembers() {
  ...
  groupMembers.value = await getGroupMemberList(call.value.groupId)
}
